...
import {
  getLearnedDiccionario, getLearnedEstados, getLearnedCiudades, getManualMaestro, getMeta,
  putLearnedEstado, putLearnedCiudad, clearLearned,
} from '@/storage/db'

// Production concern, not a hypothetical: anyone who has used the app already has an older
// database holding everything they taught the motor. Each new store bumps the version, so the
// upgrade path has to be additive — it must create the new stores WITHOUT dropping the ones that
// already exist or the meta values (fuzzy thresholds) alongside them. This test starts from v1,
// the oldest shape in the wild, so it covers every hop at once.
describe('IndexedDB v1 → v3 migration', () => {
  test('a pre-existing v1 database keeps its data and gains the new stores', async () => {
    ...
    // …and both new stores are there, usable, and cleared by resetLearned along with the rest.
    ...
    await putLearnedCiudad({ ciudad: 'EL PARAISO', estadoStd: 'DISTRITO CAPITAL', activa: true })
    expect(await getLearnedEstados()).toHaveLength(1)
    expect(await getLearnedCiudades()).toHaveLength(1)
